...
Original file is located at
    https://colab.research.google.com/drive/1lb3kxsHO0SuqWyWRrFgrQ9yQw74zPCKI

# DenseNet 이론

- Deep Network는 gradient 소실 문제가 있다. 이것을 방지하기 위해 ResNet은 잔차를 이용 & 숏컷 연결을 통해 정보가 연결 되게 한다.
- 반면에 DenseNet은 이전 특성 맵 전체를 다음 계층의 입력으로 사용한다.
- 또한 DenseNet은 특성 맵의 개수가 비효율적인 지점까지 증가하는 것을 막기 위해, 병목 계층을 사용한다.
- 병합 연산에서 Concatenation을 수행하기 위해서는 크기가 동일해야 하기 때문에, DenseNet은 심층 신경망을 여러 밀집 블록으로 나누고 전이 계층을 통해 서로 연결되게 만든다. 각 밀집 블록 내에서 특징 맵의 크기는 일정하며, 전이 계층을 거치면서 특징 맵 크기가 줄어들 수 있다.
